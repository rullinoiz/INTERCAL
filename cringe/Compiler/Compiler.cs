...
internal static class Compiler
{
    ...
    private static string PrepareSource(IEnumerable<string> files)
    {
        ...
        foreach (var file in files)
        {
            ...
            if (dot < 0)
                throw new CompilationException(IntercalError.E998 + " (" + file + ")");
            ...
            {
                Exception err = new CompilationException(IntercalError.E777 + " (" + file + ")", e);
                throw err;
            }
        }
        return src;
    ...
    private static void EmitBinary(CompilationContext c)
    {
        ...
        {
            throw new CompilationException(IntercalError.E888, e);
        }
        ...
        compilerArgs += c.TypeOfAssembly switch
        {
            CompilationContext.AssemblyType.Winexe or CompilationContext.AssemblyType.Exe => " /out:" +
                c.NameOfAssembly + ".exe ",
            CompilationContext.AssemblyType.Library => " /t:library /out:" + c.NameOfAssembly + ".dll ",
    ...
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            foreach (var arg in args)
            {
                if (arg[0] is '-' or '/')
                {
                    if (arg[1..].Equals("v", StringComparison.CurrentCultureIgnoreCase) || arg[1..].Equals("verbose", StringComparison.CurrentCultureIgnoreCase))
                        ...
                        switch (arg[3..])
                        {
                            ...
                            default: Abort(IntercalError.E2001); break;
                        }
            ...
            if (p.StatementCount > 3)
            {
                switch (p.Politeness)
                {
                    ...
                    case < EMinimumPoliteness:
                        Abort(IntercalError.E079);
                        break;
                    // more than 1/3 and you are too polite
                    case > EMaximumPoliteness:
                        Abort(IntercalError.E099);
                        break;
                }
            }
    ...
    private static string FindFile(string path)
    {
        ...
        throw new IntercalException(IntercalError.E2002);
    }
