...
{
	#region fields
	private static readonly Dictionary<Type, int> AbstainSlots = new();
	private static readonly Dictionary<Type, bool> TraditionalDict = new();
	...
	public LabelStatement OuterLabel;
	...
	public static string ReadGroupValue(Scanner s, string group)
	{
		...
		throw new ParseException(string.Format(IntercalError.E017, Scanner.LineNumber + 1));
	}
	...
	public static Statement CreateStatement(Scanner s)
	{
		...
		try
		{
			...
			if (s.Current.Groups["statement"].Success)
				...
				retval = s.Current.Value switch
				{
					AbstainStatement.Token		=> new AbstainStatement(s),
					ReadOutStatement.Token		=> new ReadOutStatement(s),
					WriteInStatement.Token		=> new WriteInStatement(s),
					ComeFromStatement.Token		=> new ComeFromStatement(s),
					ReinstateStatement.Token	=> new ReinstateStatement(s),
					StashStatement.Token		=> new StashStatement(s),
					ResumeStatement.Token		=> new ResumeStatement(s),
					ForgetStatement.Token		=> new ForgetStatement(s),
					IgnoreStatement.Token		=> new IgnoreStatement(s),
					RememberStatement.Token		=> new RememberStatement(s),
					RetrieveStatement.Token		=> new RetrieveStatement(s),
					GiveUpStatement.Token		=> new GiveUpStatement(),
					TryAgainStatement.Token		=> new TryAgainStatement(),
					_ => null
				};
		...
		if (TryAgainStatement.TryAgainLine > 0)
			throw new CompilationException(IntercalError.E993);
